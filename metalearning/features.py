...
class NumberOfMissingValues(MetaFeature):
    def _calculate(self, X, y, categorical, metafeatures, helpers):
        return float(helpers.get_value("MissingValues").sum())
...
@helpers.define("ClassOccurrences")
class ClassOccurrences(HelperFunction):
    def _calculate(self, X, y, categorical, metafeatures, helpers):
        if len(y.shape) == 2:
            occurrences = []
            for i in range(y.shape[1]):
                occurrences.append(self._calculate(X, y[:, i], categorical, metafeatures, helpers))
            return occurrences
        else:
...
@metafeatures.define("ClassProbabilityMin", dependency="ClassOccurrences")
...
@metafeatures.define("ClassProbabilityMax", dependency="ClassOccurrences")
...
@metafeatures.define("ClassProbabilityMean", dependency="ClassOccurrences")
class ClassProbabilityMean(MetaFeature):
    def _calculate(self, X, y, categorical, metafeatures, helpers):
        occurrence_dict = helpers.get_value("ClassOccurrences")
...
@metafeatures.define("ClassProbabilitySTD", dependency="ClassOccurrences")
class ClassProbabilitySTD(MetaFeature):
    def _calculate(self, X, y, categorical, metafeatures, helpers):
        ...
        if len(y.shape) == 2:
            ...
            occurrences = np.array([occurrence for occurrence in occurrence_dict.values()],
                                   dtype=np.float64)
            return (occurrences / y.shape[0]).std()
...
class SymbolsMin(MetaFeature):
    def _calculate(self, X, y, categorical, metafeatures, helpers):
        ...
        minimum = None
        for unique in helpers.get_value("NumSymbols"):
...
class SymbolsMax(MetaFeature):
    def _calculate(self, X, y, categorical, metafeatures, helpers):
        values = helpers.get_value("NumSymbols")
        if len(values) == 0:
            return 0
...
class ClassEntropy(MetaFeature):
    def _calculate(self, X, y, categorical, metafeatures, helpers):
        ...
        for i in range(labels):
            ...
            for value in y[:, i]:
                occurrence_dict[value] += 1
            entropies.append(scipy.stats.entropy([occurrence_dict[key] for key in
                                                  occurrence_dict], base=2))
