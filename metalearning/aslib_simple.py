...
class AlgorithmSelectionProblem(object):
    ...
    def _read_algorithm_runs(self, filename):
        ...
        for data in arff_dict["data"]:
            ...
            for i, performance_measure in enumerate(performance_measures):
                measure_instance_algorithm_triples[performance_measure][inst_name][algorithm] \
                    = perf_list[i]
    ...
    def _read_configurations(self, filename):
        ignore = ['idx', 'classifier:qda:tol',
                  'one_hot_encoding:minimum_fraction',
                  'one_hot_encoding:use_minimum_fraction',
                  'imputation:strategy',]
                  #'preprocessor:gem:N', 'preprocessor:gem:precond']
        ...
        with open(filename) as fh:
            ...
            for line in csv_reader:
                ...
                for hp_name, value in line.items():
                    if not value or hp_name in ignore:
                        continue
                    try:
                        value = int(value)
                    except:
    ...
    def get_config_info(self):
        config_path = os.path.join(self.dir_, "configurations.csv")
        config_info = dict()
        with open(config_path) as fh:
            csv_reader = csv.DictReader(fh)
            config_info['fields'] = csv_reader.fieldnames
            config_info['last_idx'] = deque(csv_reader, 1)[0]['idx']
